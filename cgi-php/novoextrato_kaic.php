	...
	while ($ver = ibase_fetch_assoc($resultado))
	{
		...
		if(preg_match($er,$linhas,$match))
		{
			...
			if ($listar==-1)
			{
				...
				if((@$conta==$dados[0])&&(@$senha==$dados[1])&&(@$cli_ss==$cliente)&&(@$mesinfo==$vmes))
				{
					$listar=$n;
					$listamen=$n;
					$meseano=$vmes;
		...
		if ($listar!=-1)
		{
			...
			//if(preg_match("/^([0-9]{2}/?[0-9]{0,2}\#.*)/",$linhas,$match))
			if(preg_match($er_disc,$linhas,$match))
			{
				// Preparando Coluna 2
				$texto=explode("#",$linhas);
...
<table id="relatorio">
<tr><td colspan="3" class="t_s">DEMONSTRATIVO DE RECEBIMENTOS E PAGAMENTOS<br /><?=@$nome?></td><td align="right" class="d_r" ><?=$meseano?></td></tr>
